...
void do_DiscesaBassa(){
	...
	if ((Sistema.Altitudine - Sistema.AltitudineTerra < MISSIONE_ATTIVAZIONE_BUZZER) && ((Sistema.Altitudine - vecchiaAltitudine) <= 0) && (Sistema.StatoAttuale != STATE_LANDED)){
		
		//Esegui lo staging
		Coprocessore::eseguiComando(CMD_PROC_WIND_OFF);
		Coprocessore::eseguiComando(CMD_PROC_BUZZER_ON);
		
		//Cambio fase
		Sistema.StatoPrecedente = Sistema.StatoAttuale;
		Sistema.StatoAttuale = STATE_LANDED;
	}
